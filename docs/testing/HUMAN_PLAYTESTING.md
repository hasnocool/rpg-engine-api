...
**Execution policy:** canonical profiles are executed by the designated local test agent only; GitHub Actions and remote CI are prohibited by `PLAN.md` Section 50.
...
The harness should be usable against local execution targets such as:
...
The exact transport may vary, but the gameplay path must remain the public API/live protocol. Remote CI is not an execution target under the repository policy.
...
Provide explicit **local runner modes**.
...
All scenarios affected by changed feature tags plus mandatory core regressions. This is a local pre-merge profile, not GitHub Actions.
...
Full suite plus broader timing/spatial/persona matrices and generated walkers. It may be run manually or by a user-controlled local scheduler; it is not a GitHub Actions schedule.
...
# 19. Local selection and feature tagging
...
Changed code should map to affected feature tags so the local `pr` profile can select relevant scenarios while preserving a mandatory core smoke/regression set.

Do not rely exclusively on selective testing; broader local `full`, `nightly`, and `release` profiles remain required at the appropriate gates.

Do not create or use GitHub Actions workflows to perform this selection or schedule these profiles.
...
Artifacts are generated by the local test agent. GitHub Actions artifacts are not used.
...
The full 76-step acceptance flow in `PLAN.md` must exist as executable scenario(s), not just prose. Release gating must prove it programmatically through public interfaces with local `release` evidence.
...
Which local profile proves it on the candidate commit?
```

If those questions cannot be answered, the feature is not complete enough to merge as a finished gameplay capability.

GitHub Actions must not be created or used to satisfy this requirement.