...
from rpg_engine_api.api.intelligence import router as intelligence_router
...
from rpg_engine_api.api.portable import router as portable_router
...
def create_app(settings: Settings | None = None) -> FastAPI:
    resolved = settings or get_settings(); store = PostgresEventStore(resolved.database_url) if resolved.postgres_configured and resolved.database_url else None; engine = ProductionReleaseEngineService(store=store)
    @asynccontextmanager
    ...
    for router in (health_router, command_router, query_router, creator_router, evolution_router, advanced_router, composition_router, living_world_router, extension_router, intelligence_router, discovery_router, portable_router, production_router, observability_router, ws_router): app.include_router(router)
    return app
