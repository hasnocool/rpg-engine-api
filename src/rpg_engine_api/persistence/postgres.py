...
class PostgresEventStore:
    ...
    async def current_version(self, stream_id: str) -> int:
        async with self.engine.connect() as connection:
            value = await connection.scalar(
                select(event_streams.c.version).where(event_streams.c.stream_id == stream_id)
            )
        ...
        async with self.engine.begin() as connection:
            row = (
                await connection.execute(
                    select(event_streams.c.version)
                    .where(event_streams.c.stream_id == stream_id)
                    .with_for_update()
                )
            ).first()
            if row is None:
            ...
            for raw in pending:
                ...
                result = await connection.execute(
                    ...
                    .values(
                        ...
                        event_json=provisional.model_dump_json(),
                    )
                ...
                await connection.execute(
                    ...
                    .values(event_json=event.model_dump_json())
                )
                await connection.execute(
    ...
    async def read_stream(self, stream_id: str) -> tuple[DomainEvent, ...]:
        async with self.engine.connect() as connection:
            rows = (
                await connection.execute(
                    select(domain_events.c.event_json)
                    .where(domain_events.c.stream_id == stream_id)
                    .order_by(domain_events.c.stream_version)
                )
            ).scalars()
    ...
    async def read_all(self) -> tuple[DomainEvent, ...]:
        async with self.engine.connect() as connection:
            rows = (
                await connection.execute(
                    select(domain_events.c.event_json).order_by(domain_events.c.sequence)
                )
            ).scalars()
            return tuple(DomainEvent.model_validate_json(value) for value in rows)
    ...
    async def get_receipt(self, key: str) -> CommandReceipt | None:
        async with self.engine.connect() as connection:
            value = await connection.scalar(
                select(command_receipts.c.receipt_json).where(
                    command_receipts.c.idempotency_key == key
                )
            )
    ...
    async def save_receipt(self, key: str, receipt: CommandReceipt) -> None:
        async with self.engine.begin() as connection:
            existing = await connection.scalar(
                select(command_receipts.c.idempotency_key).where(
                    command_receipts.c.idempotency_key == key
                )
            )
            if existing is None:
