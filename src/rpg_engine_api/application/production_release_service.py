...
class ProductionReleaseEngineService(ExtensionEngineService):
    ...
    async def _dry_run_content_revision(self, command: CommandEnvelope, principal: PrincipalContext) -> CommandReceipt:
        ...
        try:
            ...
            success = bool(
                sandbox_report["pre_replay_matches_live"]
                and sandbox_report["target_quality"]["valid"]
                and sandbox_report["post_replay_matches_live"]
            )
    ...
    async def _import_character_package(self, command: CommandEnvelope, principal: PrincipalContext) -> CommandReceipt:
        ...
        if species or background:
            events.append(
                DomainEvent(
                    event_type="CharacterOriginApplied",
                    campaign_id=campaign_id,
                    stream_id=stream,
                    actor_id=actor_id,
                    command_id=command.command_id,
                    correlation_id=command.command_id,
                    payload={"species": species or "human", "background": background or "wanderer", "features": [], "items": []},
                )
            )
        if data.get("class_id"):
            events.append(
                DomainEvent(
                    event_type="CharacterBuildApplied",
                    campaign_id=campaign_id,
                    stream_id=stream,
                    actor_id=actor_id,
                    command_id=command.command_id,
                    ...
                    payload={
                        ...
                        "known_abilities": list(data.get("known_abilities", [])),
                        "prepared_abilities": list(data.get("prepared_abilities", [])),
                        "items": [],
                        "equipment": {},
                        "resources": {},
                        "attack_bonus_delta": 0,
                        "defense_delta": 0,
                    },
                )
            )
    ...
    def capability_projection(cls) -> dict[str, Any]:
        ...
        data["features"] = list(data.get("features", [])) + [
            "admin_audit_log",
            "portable_campaign_package",
            "portable_character_package",
            "data_only_import_validation",
            "isolated_migration_sandbox",
        ]
