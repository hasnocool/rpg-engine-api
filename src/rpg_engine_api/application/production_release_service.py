...
from rpg_engine_api.application.intelligent_extension_service import IntelligentExtensionEngineService
...
class ProductionReleaseEngineService(IntelligentExtensionEngineService):
    """Top-level shipping/operations layer above the complete gameplay service chain."""
    OWNER_COMMANDS = IntelligentExtensionEngineService.OWNER_COMMANDS | {"ImportCharacterPackage"}

    async def execute(self, command: CommandEnvelope, principal: PrincipalContext, *, drive_controllers: bool = True) -> CommandReceipt:
        receipt = await super().execute(command, principal, drive_controllers=drive_controllers); metrics = getattr(self, "metrics", None)
        if metrics is not None:
            if "controller" in principal.roles and command.command_type == "PerformAction": metrics.record_controller_decision()
            triggered = receipt.result.get("triggered_scheduled_events")
            if isinstance(triggered, list): metrics.record_scheduler_events(len(triggered))
            if command.command_type.startswith("Simulate"): metrics.record_simulation_runs(int(receipt.result.get("runs", 1)))
        try: await self._append_audit(command, principal, receipt)
        except Exception:
            logger.exception("audit_record_failed command_id=%s", command.command_id)
            if metrics is not None: metrics.record_operational_failure("audit")
        return receipt
    ...
    async def _dry_run_content_revision(self, command: CommandEnvelope, principal: PrincipalContext) -> CommandReceipt:
        ...
        try:
            sandbox_report = await run_content_migration_sandbox(self, proposal_id, type(self)); success = bool(sandbox_report["pre_replay_matches_live"] and sandbox_report["target_quality"]["valid"] and sandbox_report["post_replay_matches_live"]); return receipt.model_copy(update={"result": {**receipt.result, "sandbox_report": sandbox_report}})
        finally:
            metrics = getattr(self, "metrics", None)
            if metrics is not None: metrics.record_migration_dry_run(success=success)

    async def _append_audit(self, command: CommandEnvelope, principal: PrincipalContext, receipt: CommandReceipt) -> None:
        stream = "system:audit"; payload = {"principal_id": principal.principal_id, "roles": sorted(principal.roles), "command_type": command.command_type, "campaign_id": command.campaign_id, "actor_id": command.actor_id, "status": receipt.status.value, "error_code": receipt.error.code.value if receipt.error else None, "request": redact(command.model_dump(mode="json")), "result": redact(receipt.result)}
        event = DomainEvent(event_type="AdminAuditRecorded", campaign_id="__system__", stream_id=stream, command_id=command.command_id, correlation_id=command.command_id, payload=payload); await self.store.append(stream, await self.store.current_version(stream), (event,))

    async def audit_records(self, *, limit: int = 200, principal_id: str | None = None, campaign_id: str | None = None) -> list[dict[str, Any]]:
        events = await self.store.read_stream("system:audit"); records: list[dict[str, Any]] = []
        for event in reversed(events):
            payload = dict(event.payload)
            if principal_id is not None and payload.get("principal_id") != principal_id: continue
            if campaign_id is not None and payload.get("campaign_id") != campaign_id: continue
            records.append({"audit_id": event.event_id, "sequence": event.sequence, "timestamp": event.server_timestamp.isoformat(), **payload})
            if len(records) >= max(1, min(limit, 1000)): break
        return records
    ...
    async def export_campaign_package(self, campaign_id: str) -> PortableCampaignPackage:
        ...
        binding = self.campaign_content_bindings.get(campaign_id); packs = ()
        if binding is not None: packs = tuple(pack for (pack_id, _), pack in self.published_packs.items() if pack_id == binding.pack_id)
        return PortableCampaignPackage.from_backup(await export_event_history(self.store, campaign_id=campaign_id, content_packs=packs))
    ...
    async def _import_character_package(self, command: CommandEnvelope, principal: PrincipalContext) -> CommandReceipt:
        ...
        if not package.verify(): raise ValueError("character package failed integrity/security validation")
        data = package.character; actor_id = str(command.payload.get("actor_id") or new_id("act")); actor_receipt = await self._create_actor(CommandEnvelope(command_id=new_id("cmd"), command_type="CreateActor", campaign_id=campaign_id, payload={"actor_id": actor_id, "name": str(data.get("name") or "Imported Character"), "max_hp": int(data.get("max_hp", 10)), "attack_bonus": int(data.get("attack_bonus", 2)), "defense": int(data.get("defense", 10)), "controller": {"controller_type": "human", "controller_version": "1"}}), principal)
        actor = self.actors[actor_id]; stream = f"actor:{actor_id}"; expected = await self.store.current_version(stream); events: list[DomainEvent] = []; species = data.get("species"); background = data.get("background")
        if species or background: events.append(DomainEvent(event_type="CharacterOriginApplied", campaign_id=campaign_id, stream_id=stream, actor_id=actor_id, command_id=command.command_id, correlation_id=command.command_id, payload={"species": species or "human", "background": background or "wanderer", "features": [], "items": []}))
        if data.get("class_id"): events.append(DomainEvent(event_type="CharacterBuildApplied", campaign_id=campaign_id, stream_id=stream, actor_id=actor_id, command_id=command.command_id, correlation_id=command.command_id, payload={"class_id": str(data["class_id"]), "subclass_id": data.get("subclass_id"), "ability_scores": dict(data.get("ability_scores", {})), "proficiencies": list(data.get("proficiencies", [])), "known_abilities": list(data.get("known_abilities", [])), "prepared_abilities": list(data.get("prepared_abilities", [])), "items": [], "equipment": {}, "resources": {}, "attack_bonus_delta": 0, "defense_delta": 0}))
        if events:
            stored = await self.store.append(stream, expected, events); state = actor
            for event in stored: state = reduce_actor(state, event)
